...
class Validata
{
    ...
    // ------------------------
    const E_IS_EMPTY 		= 	'输入项为空';
    const E_VALID_EMAIL		=	'无效的邮件地址';
    const E_VALID_URL		=	'无效的链接';
    const E_ISSAME			=	'不匹配';
    const E_VALID_IP		=	'无效的ip地址';
    const E_ALPHA			=	'仅支持字母';
    const E_ALPHANUIMERIC	=	'仅支持数字和字母';
    const E_INT 			=	'不是整形数字';
    const E_FLOAT 			=	'不是浮点数字';
    const E_MINLENGTH		=	'不满足最小字符长度';
    const E_MAXLENGTH		=	'超过最大字符长度';
